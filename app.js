...
const audioFile=$("audioFile"),audio=$("audio"),lyrics=$("lyrics"),loadLyrics=$("loadLyrics"),clearAll=$("clearAll"),playPause=$("playPause"),back3=$("back3"),forward3=$("forward3"),now=$("now"),durationEl=$("duration"),progressText=$("progressText"),currentPhrase=$("currentPhrase"),nextPhrase=$("nextPhrase"),tapButton=$("tapButton"),undo=$("undo"),skip=$("skip"),latency=$("latency"),latencyValue=$("latencyValue"),lastMinus=$("lastMinus"),lastPlus=$("lastPlus"),exportLrc=$("exportLrc"),exportSrt=$("exportSrt"),exportCsv=$("exportCsv"),exportJson=$("exportJson"),importJson=$("importJson"),output=$("output"),list=$("list"),tapSound=$("tapSound");
let state={lines:[],times:[],index:0,latencyMs:-120};
...
function saveLocal(){localStorage.setItem("lyricTapperProjectV3",JSON.stringify({lyricText:lyrics.value,state}))}
function loadLocal(){const raw=localStorage.getItem("lyricTapperProjectV3");if(!raw)return;try{const d=JSON.parse(raw);lyrics.value=d.lyricText||"";if(d.state)state=d.state;latency.value=state.latencyMs??-120;update()}catch{}}
function update(){state.latencyMs=Number(latency.value);latencyValue.textContent=String(state.latencyMs);const total=state.lines.length,done=state.times.filter(t=>typeof t==="number").length;progressText.textContent=`${done} / ${total}`;
 if(!total){currentPhrase.textContent="歌詞をセットしてください";nextPhrase.textContent="---"}
 else if(state.index>=total){currentPhrase.textContent="打刻完了！";nextPhrase.textContent="出力できます"}
 else{currentPhrase.textContent=state.lines[state.index];nextPhrase.textContent=state.lines[state.index+1]||"最後のフレーズです"}
...
function playTapBeep(){
 if(!tapSound||!tapSound.checked)return;
 try{
  ...
  gain.gain.exponentialRampToValueAtTime(0.28,audioCtx.currentTime+0.006);
  gain.gain.exponentialRampToValueAtTime(0.0001,audioCtx.currentTime+0.075);
  osc.connect(gain);
...
exportJson.onclick=()=>download("lyrics_tapper_project.json",JSON.stringify({app:"歌詞ハイ！タイマー",version:3,lyricText:lyrics.value,state},null,2),"application/json");
importJson.onchange=async()=>{const f=importJson.files[0];if(!f)return;try{const d=JSON.parse(await f.text());lyrics.value=d.lyricText||d.tapText||"";if(d.state){if(d.state.lines)state=d.state;else if(d.state.tapLines)state={lines:d.state.displayLines?.length===d.state.tapLines?.length?d.state.displayLines:d.state.tapLines,times:d.state.times||[],index:d.state.index||0,latencyMs:d.state.latencyMs??-120}}latency.value=state.latencyMs??-120;update()}catch{alert("JSONを読み込めませんでした")}};
clearAll.onclick=()=>{if(!confirm("作業データを消しますか？"))return;state={lines:[],times:[],index:0,latencyMs:-120};lyrics.value="";latency.value=-120;localStorage.removeItem("lyricTapperProjectV3");update()};

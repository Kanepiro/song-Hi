...
function update(){normalizeState();state.latencyMs=Number(latency.value);latencyValue.textContent=String(state.latencyMs);audio.playbackRate=state.rate||1;updateSpeedButtons();const total=state.lines.length,done=state.times.filter(t=>typeof t==="number").length,clears=state.clearTimes.length;progressText.textContent=`${done} / ${total}${clears?`　消 ${clears}`:""}`;
 if(!total){prevPhrase.textContent="---";currentPhrase.textContent="歌詞をセットしてください";nextPhrase.textContent="---";nextNextPhrase.textContent="次の次：---"}
 else if(state.index>=total){prevPhrase.textContent=state.lines[total-1]||"---";currentPhrase.textContent="打刻完了！";nextPhrase.textContent="出力できます";nextNextPhrase.textContent="次の次：---"}
 else{prevPhrase.textContent=state.index>0?state.lines[state.index-1]:"最初のフレーズです";currentPhrase.textContent=state.lines[state.index];nextPhrase.textContent=state.lines[state.index+1]||"最後のフレーズです";nextNextPhrase.textContent="次の次："+(state.lines[state.index+2]||"---")}
...
exportJson.onclick=()=>download("lyrics_tapper_project.json",JSON.stringify({app:"歌詞ハイ！タイマー",version:7,lyricText:lyrics.value,state},null,2),"application/json");
...
clearAll.onclick=()=>{if(!confirm("作業データを消しますか？"))return;state={lines:[],times:[],clearTimes:[],index:0,latencyMs:-120,rate:1};lyrics.value="";latency.value=-120;localStorage.removeItem("lyricTapperProjectV7");localStorage.removeItem("lyricTapperProjectV6");localStorage.removeItem("lyricTapperProjectV5");update()};

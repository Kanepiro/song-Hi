...
exportJson.onclick=()=>download("lyrics_tapper_project.json",JSON.stringify({app:"歌詞ハイ！タイマー",version:14,lyricText:lyrics.value,state},null,2),"application/json");
...
clearAll.onclick=()=>{if(!confirm("作業データを消しますか？"))return;state={lines:[],times:[],clearTimes:[],index:0,latencyMs:-120,rate:1};lyrics.value="";latency.value=-120;localStorage.removeItem("lyricTapperProjectV14");localStorage.removeItem("lyricTapperProjectV13");localStorage.removeItem("lyricTapperProjectV10");localStorage.removeItem("lyricTapperProjectV9");localStorage.removeItem("lyricTapperProjectV7");localStorage.removeItem("lyricTapperProjectV6");localStorage.removeItem("lyricTapperProjectV5");update()};
